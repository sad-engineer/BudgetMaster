    ...
    public void deleteExpensesSoft(List<Operation> expenses) {
        ...
        for (Operation expense : expenses) {
            if (!expense.isDeleted()) {
                ...
                Log.w(TAG, "Пропуск операции: ID=" + expense.getId() + ", уже удалена");
            }
        }
        ...
        Log.d(TAG, "Начато удаление операций. Количество: " + expensesToDelete.size());
        ...
        ioExecutor.execute(() -> {
            ...
            for (Operation expense : expensesToDelete) {
                try {
                    Log.d(TAG, "Удаление операции: ID=" + expense.getId());
                    // TODO: Реализовать метод delete в OperationService
                    // operationService.delete(true, expense);
                    deletedCount++;
                    Log.d(TAG, "Операция ID: " + expense.getId() + " успешно удалена");
                } catch (Exception e) {
                    Log.e(TAG, "Ошибка удаления операции: ID=" + expense.getId() + ", причина: " + e.getMessage());
                }
            }
     ...
     public void setSelectedExpenses(List<Operation> expenses) {
        Log.d(TAG, "Выбранных операций: " + (expenses != null ? expenses.size() : 0));
