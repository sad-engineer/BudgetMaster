...
public class FinancialCalculationCoordinator {
    ...
    public BudgetSummary getBudgetSummary() {
        ...
        CurrencyService currencyService = sm.currencies.service();
        BudgetService budgetService = sm.budgets.service();

        List<Integer> currencyIds = currencyService.getAvalibleIds(EntityFilter.ACTIVE);
        if (currencyIds == null || currencyIds.isEmpty()) {
            Log.d(TAG, "Список доступных ID валют пуст");
            return new BudgetSummary();
        }
        for (int currencyId : currencyIds) {
            amount += budgetService.getTotalAmountByCurrency(currencyId, EntityFilter.ACTIVE);
        }

        return new BudgetSummary(amount);
    }
